# -*- coding: utf-8 -*-
"""
Created on Tue Feb  1 16:30:38 2022
...
boundaries = 'outer_wave.csv'  # File generated from the bnd coordinates script

# nc_file_req = input('Enter the NetCDF file name : ')
nc_file = '2015_1000m_wave_2D.nc'
...
end_time = '2015-02-14 00:00:00'
# TODO: automate extraction # this is used to slice the nc_file

# step_req = input(
#    'Enter time step to extract data (max resolution is 20mins) in format 2.0000000e+001 : ')
...
#  one_time_step_bcw = float(value_from_txt_file(mdw_file, 'Time'))
...
if coupled_or_not == 'True':
    one_time_step_bcw = value_from_txt_file(mdf_file, 'Flpp')  # coupling interval
    one_time_step_bcw = one_time_step_bcw.split(' ')
    one_time_step_bcw = float(one_time_step_bcw[1].strip())
else:
    one_time_step_bcw = float(input('Enter time interval between calculations : '))
...
for key in converted_dataset_dict:
    ...
    with open(bcw_file, 'a', newline='') as f:
